...
select_eks_cluster() {
    if [ "$ENV" == "dev" ] || [ "$ENV" == "qa" ]; then
        ...
        aws eks update-kubeconfig --name OhEKSDevInternal --region us-east-2 --role-arn arn:aws:iam::532968567499:role/admin-role-OhEKSDevInternal
    elif [ "$ENV" == "prod" ]; then
        if [ "$REGION" == "us-east-2" ]; then
            echo "Logging in Prod EKS in US Region"
            aws eks update-kubeconfig --name OhEKSIntApis --region us-east-2 --role-arn arn:aws:iam::532968567499:role/admin-role-OhEKSIntApis
        ...
        aws eks update-kubeconfig --name OhEKSIntApiPci --region us-east-2 --role-arn arn:aws:iam::532968567499:role/admin-role-OhEKSIntApiPci
    else
